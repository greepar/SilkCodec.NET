...
internal static class ControlCodecFLP {
     ...
     * @return
     */
    internal static int SKP_Silk_control_encoder_FLP(
        SKP_Silk_encoder_state_FLP  psEnc,              /* I/O  Pointer to Silk encoder state FLP       */
        int                   API_fs_Hz,          /* I    External (API) sampling rate (Hz)       */
        int                   max_internal_fs_kHz,/* I    Maximum internal sampling rate (kHz)    */
        int                   PacketSize_ms,      /* I    Packet length (ms)                      */
              int                   TargetRate_bps,     /* I    Target max bitrate (if SNR_dB == 0)     */
        int                   PacketLoss_perc,    /* I    Packet loss rate (in percent)           */
        int                   INBandFEC_enabled,  /* I    Enable (1) / disable (0) inband FEC     */
        int                   DTX_enabled,        /* I    Enable / disable DTX                    */
        int                   InputFramesize_ms,  /* I    Inputframe in ms                        */
        int                   Complexity          /* I    Complexity (0->low; 1->medium; 2->high) */
    )
    {
            ...
            { /* Low speech activity and payload buffer empty */
                ...
                if(SWITCH_TRANSITION_FILTERING!=0)
                {
                    ...
                    {                                       /* Ready to switch down */
                        ...
                        {
                            EncoderCompat.Assert( psEnc.sCmn.fs_kHz == 12 );
                            fs_kHz = 8;
                        }
                    }
                }
    ...
    //    #endif
                        ...
                        {
                            EncoderCompat.Assert( psEnc.sCmn.fs_kHz == 16 );
                            fs_kHz = 24;
                        }
                    }
                }
    ...
    //    #endif
        ...
        if(SWITCH_TRANSITION_FILTERING != 0)
        {
                ...
                ( psEnc.sCmn.nFramesInPayloadBuf == 0 ) )
            {
                ...
                EncoderCompat.Fill(psEnc.sCmn.sLP.In_LP_State, 0, 2, 0);
            }
        }
        ...
        if( psEnc.sCmn.fs_kHz != fs_kHz || psEnc.sCmn.prev_API_fs_Hz != API_fs_Hz )
        {
            ...
            {
                /* Copy data */
                Array.Copy(x_bufFIX, 0, x_buf_API_fs_Hz, 0, nSamples_temp);
            }
...
//TODO: psEnc.sNSQ.vq is of short[], why sizeof(SKP_float)???
            EncoderCompat.Fill(psEnc.sNSQ.xq, 0, 2 * MAX_FRAME_LENGTH, (short)0);
//            SKP_memset( psEnc->sNSQ_LBRR.xq,     0, ( 2 * MAX_FRAME_LENGTH ) * sizeof( SKP_float ) );
            EncoderCompat.Fill(psEnc.sNSQ_LBRR.xq, (short)0);
...
//            SKP_memset( psEnc->sPred.prev_NLSFq, 0, MAX_LPC_ORDER * sizeof( SKP_float ) );
            EncoderCompat.Fill(psEnc.sPred.prev_NLSFq, 0, MAX_LPC_ORDER, 0);
...
//                psEnc.psNLSF_CB_FLP[  1 ]  = &SKP_Silk_NLSF_CB1_10_FLP;
            ...
            EncoderCompat.Assert( ( psEnc.sCmn.subfr_length * NB_SUBFR ) == psEnc.sCmn.frame_length );
        }
...
//    #endif
     ...
     */
    internal static void SKP_Silk_LBRR_ctrl_FLP(
 ...
 * @author Dingxin Xu
 */
internal sealed class NSQImplNSQ : NoiseShapingQuantizerFP
{
 ...
 * @author Dingxin Xu
 */
internal sealed class NSQImplNSQDelDec : NoiseShapingQuantizerFP
{
    public void NoiseShapingQuantizer(SKP_Silk_encoder_state psEnc,
            SKP_Silk_encoder_control psEncCtrl, SKP_Silk_nsq_state NSQ,
            short[] x, byte[] q, int arg6, short[] arg7, short[] arg8,
