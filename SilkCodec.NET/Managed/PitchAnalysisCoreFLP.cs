...
using static SilkCodec.NET.Managed.PitchEstDefinesFLP;
...
internal static class PitchAnalysisCoreFLP
{
    internal const int SCRATCH_SIZE =   22;
     ...
     * @param search_thres2 threshold for lag candidates 0 - 1
     ...
     */
    internal static int SKP_Silk_pitch_analysis_core_FLP( /* O voicing estimate: 0 voiced, 1 unvoiced                 */
    ...
    )
    {
        ...
        EncoderCompat.Assert( Fs_kHz == 8 || Fs_kHz == 12 || Fs_kHz == 16 || Fs_kHz == 24 );
        ...
        EncoderCompat.Assert( search_thres1 >= 0.0f && search_thres1 <= 1.0f );
        EncoderCompat.Assert( search_thres2 >= 0.0f && search_thres2 <= 1.0f );
        ...
        else if( Fs_kHz == 16 )
        {
            ...
            {
                EncoderCompat.Assert( 4 <= PITCH_EST_MAX_DECIMATE_STATE_LENGTH );
                for(int i_djinn=0; i_djinn<4; i_djinn++)
                    filt_state[i_djinn] = 0;

                Decimate2CoarseFLP.SKP_Silk_decimate2_coarse_FLP( signal,0, filt_state,0, signal_8kHz,0,
                    scratch_mem,0, frame_length_8kHz );
            }
            else
            {
                EncoderCompat.Assert( 2 <= PITCH_EST_MAX_DECIMATE_STATE_LENGTH );
                for(int i_djinn=0; i_djinn<2; i_djinn++)
                    filt_state[i_djinn] = 0;

                Decimate2CoarsestFLP.SKP_Silk_decimate2_coarsest_FLP( signal,0, filt_state,0, signal_8kHz,0,
                    scratch_mem,0, frame_length_8kHz );
            }
        }
        else if( Fs_kHz == 24 )
        ...
        ******************************************************************************/
        ...
        for( k = 0; k < 2; k++ )
        {
            ...
            EncoderCompat.Assert( target_ptr_offset >= 0 );
            EncoderCompat.Assert( target_ptr_offset + sf_length_8kHz <= frame_length_4kHz );

            basis_ptr = target_ptr;
            basis_ptr_offset = target_ptr_offset - min_lag_4kHz;

            /* Check that we are within range of the array */
            EncoderCompat.Assert( basis_ptr_offset >= 0 );
            EncoderCompat.Assert( basis_ptr_offset + sf_length_8kHz <= frame_length_4kHz );
...
//                        basis_ptr[ sf_length_8kHz ] * basis_ptr[ sf_length_8kHz ];
        ...
        EncoderCompat.Assert( length_d_srch <= PITCH_EST_D_SRCH_LENGTH );
        ...
        EncoderCompat.Assert( length_d_srch > 0 );
        ...
        for( i = max_lag_8kHz + 3; i >= min_lag_8kHz; i-- ) {
            d_comp[ i ] = unchecked((short)(d_comp[ i ] + d_comp[ i - 1 ] + d_comp[ i - 2 ]));
        }
        ...
        /* Convolution */
        for( i = max_lag_8kHz + 3; i >= min_lag_8kHz; i-- ) {
            d_comp[ i ] = unchecked((short)(d_comp[ i ] + d_comp[ i - 1 ] + d_comp[ i - 2 ] + d_comp[ i - 3 ]));
        }
        ...
        *********************************************************************************/
        ...
        for( k = 0; k < PITCH_EST_NB_SUBFR; k++ )
        {
            /* Check that we are within range of the array */
            EncoderCompat.Assert( target_ptr_offset >= 0 );
            EncoderCompat.Assert( target_ptr_offset + sf_length_8kHz <= frame_length_8kHz );
            ...
            for( j = 0; j < length_d_comp; j++ )
            {
                ...
                /* Check that we are within range of the array */
                EncoderCompat.Assert( basis_ptr_offset >= 0 );
                EncoderCompat.Assert( basis_ptr_offset + sf_length_8kHz <= frame_length_8kHz );
        ...
        if( Fs_kHz > 8 ) {
            ...
            EncoderCompat.Assert( CCmax >= 0.0f );
        ...
        EncoderCompat.Assert( lagIndex[0] >= 0 );
        /* return as voiced */
        return 0;
    }
     ...
     */
    internal static void SKP_P_Ana_calc_corr_st3
    (
         ...
         **********************************************************************/
    {
        ...
        for( k = 0; k < PITCH_EST_NB_SUBFR; k++ )
        {
            ...
            for( j = PitchEstTables.SKP_Silk_Lag_range_stage3[ complexity ][ k ][ 0 ]; j <= PitchEstTables.SKP_Silk_Lag_range_stage3[ complexity ][ k ][ 1 ]; j++ )
            {
                ...
                EncoderCompat.Assert( lag_counter < SCRATCH_SIZE );
            ...
            for( i = cbk_offset; i < ( cbk_offset + cbk_size ); i++ )
            {
                ...
                {
                    EncoderCompat.Assert( idx + j < SCRATCH_SIZE );
                    EncoderCompat.Assert( idx + j < lag_counter );
     ...
     * @param complexity Complexity setting.
     */
    internal static void SKP_P_Ana_calc_energy_st3
    (
        float[][][] energies_st3,
        float[] signal,           /* I vector to correlate                                            */
        int signal_offset,
    ...
    ****************************************************************/
    {
        ...
        for( k = 0; k < PITCH_EST_NB_SUBFR; k++ )
        {
            ...
            EncoderCompat.Assert( energy >= 0.0 );
            ...
            for( i = cbk_offset; i < ( cbk_offset + cbk_size ); i++ )
            {
                ...
                for(j = 0; j < PITCH_EST_NB_STAGE3_LAGS; j++)
                {
                    ...
                    EncoderCompat.Assert( energies_st3[ k ][ i ][ j ] >= 0.0f );
                }
            }
            target_ptr_offset += sf_length;
        }
