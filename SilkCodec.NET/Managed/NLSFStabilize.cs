...
internal class NLSFStabilize
{
     ...
     * @param NLSF_Q15 Unstable/stabilized normalized LSF vector input Q15 [L].
     * @param NLSF_Q15_offset offset of valid data.
     * @param NDeltaMin_Q15 Normalized delta min vector input Q15, NDeltaMin_Q15[L] must be >= 1 [L+1].
     * @param L Number of NLSF parameters input the input vector.
     */
    internal static void SKP_Silk_NLSF_stabilize(
              int    []NLSF_Q15,            /* I/O:  Unstable/stabilized normalized LSF vector input Q15 [L]                    */
              int      NLSF_Q15_offset,
              int    []NDeltaMin_Q15,       /* I:    Normalized delta min vector input Q15, NDeltaMin_Q15[L] must be >= 1 [L+1] */
       int      L                    /* I:    Number of NLSF parameters input the input vector                           */
    )
    {
        ...
        System.Diagnostics.Debug.Assert( NDeltaMin_Q15[L] >= 1 );
        ...
        for( loops = 0; loops < MAX_LOOPS; loops++ ) {
            ...
            if( I == 0 ) {
                ...
                center_freq_Q15 = SigProcFIX.SKP_LIMIT_32( SigProcFIX.SKP_RSHIFT_ROUND( NLSF_Q15[ NLSF_Q15_offset + I-1] + NLSF_Q15[ NLSF_Q15_offset + I], 1 ),
        ...
        if( loops == MAX_LOOPS )
        {
            ...
            NLSF_Q15[ NLSF_Q15_offset + L-1] = SigProcFIX.SKP_min_int( NLSF_Q15[ NLSF_Q15_offset + L-1], (1<<15) - NDeltaMin_Q15[L] );
     ...
     * @param NLSF_Q15 Unstable/stabilized normalized LSF vectors input Q15 [LxN].
     * @param NDeltaMin_Q15 Normalized delta min vector input Q15, NDeltaMin_Q15[L] must be >= 1 [L+1].
     * @param N Number of input vectors to be stabilized.
     * @param L NLSF vector dimension.
     */
    internal static void SKP_Silk_NLSF_stabilize_multi(
              int        []NLSF_Q15,        /* I/O:  Unstable/stabilized normalized LSF vectors input Q15 [LxN]                 */
              int        []NDeltaMin_Q15,   /* I:    Normalized delta min vector input Q15, NDeltaMin_Q15[L] must be >= 1 [L+1] */
        int         N,               /* I:    Number of input vectors to be stabilized                                */
        int         L                /* I:    NLSF vector dimension                                                   */
    )
    {
        int n;
