 ...
 * @author Dingxin Xu
 */
internal class NLSF2A
{
    /**
     * helper function for NLSF2A(..).
     * @param output intermediate polynomial, Q20.
     ...
     */
    internal static void SKP_Silk_NLSF2A_find_poly(
            int    []output,       /* o    intermediate polynomial, Q20            */
        ...
        )
    {
        ...
        output[0] = ( 1 << 20 );
        output[1] = -cLSF[ cLSF_offset + 0];
        for( k = 1; k < dd; k++ ) {
            ftmp = cLSF[cLSF_offset + 2*k];            // Q20
            int test = ftmp * output[k];
            long test2 =  SigProcFIX.SKP_SMULL(ftmp, output[k]);

            output[k+1] = ( output[k-1] << 1 ) - (int)SigProcFIX.SKP_RSHIFT_ROUND64( SigProcFIX.SKP_SMULL( ftmp , output[k] ), 20 );

            for( n = k; n > 1; n-- ) {
                output[n] += output[n-2] - (int)SigProcFIX.SKP_RSHIFT_ROUND64( SigProcFIX.SKP_SMULL( ftmp , output[n-1] ), 20 );
            }
            output[1] -= ftmp;
        }
    }
     ...
     * @param a monic whitening filter coefficients input Q12,  [d].
     * @param NLSF normalized line spectral frequencies input Q15, [d].
        ...
        )
    {
        ...
        System.Diagnostics.Debug.Assert(SigProcFIX.LSF_COS_TAB_SZ_FIX == 128);

        /* convert LSFs to 2*cos(LSF(i)), using piecewise linear curve from table */
        for( k = 0; k < d; k++ ) {
            System.Diagnostics.Debug.Assert(NLSF[k] >= 0 );
            System.Diagnostics.Debug.Assert(NLSF[k] <= 32767 );
            ...
            System.Diagnostics.Debug.Assert(f_int >= 0);
            System.Diagnostics.Debug.Assert(f_int < SigProcFIX.LSF_COS_TAB_SZ_FIX );
        ...
        for( k = 0; k < dd; k++ ) {
            ...
            /* the Ptmp and Qtmp values at this stage need to fit input int32 */
