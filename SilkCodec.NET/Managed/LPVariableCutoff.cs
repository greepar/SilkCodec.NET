 ...
 * @author Dingxin Xu
 */
internal static class LPVariableCutoff
{
    /**
     ...
     * @param fac_Q16
     */
    internal static void SKP_Silk_LP_interpolate_filter_taps(
        int[] B_Q28, int[] A_Q28,
        int     ind,
        int     fac_Q16
    )
    {
        ...
        if( ind < TRANSITION_INT_NUM - 1 )
        {
            if( fac_Q16 > 0 )
            {
                ...
                { /* fac_Q16 is input range of a 16-bit int */
                ...
                { /* Neither fac_Q16 nor ( ( 1 << 16 ) - fac_Q16 ) is input range of a 16-bit int */
                ...
                { /* ( ( 1 << 16 ) - fac_Q16 ) is input range of a 16-bit int */

                    Debug.Assert( ( ( 1 << 16 ) - fac_Q16 ) == SigProcFIX.SKP_SAT16( ( ( 1 << 16 ) - fac_Q16) ) );
                    /* Piece-wise linear interpolation of B and A */
                    for( nb = 0; nb < TRANSITION_NB; nb++ )
                    {
                        B_Q28[ nb ] = SKP_SMLAWB(
     ...
     * @param output Low-pass filtered output signal
    ...
    internal static void SKP_Silk_LP_variable_cutoff(
        SKP_Silk_LP_state               psLP,          /* I/O  LP filter state                     */
        short[] output,           /* O    Low-pass filtered output signal     */
        int out_offset,
    ...
    )
    {
        ...
        Debug.Assert( psLP.transition_frame_no >= 0 );
        Debug.Assert( ( ( psLP.transition_frame_no <= TRANSITION_FRAMES_DOWN ) && ( psLP.mode == 0 ) ) ||
        ...
        if( psLP.transition_frame_no > 0 )
        {
            if( psLP.mode == 0 )
            {
                if( psLP.transition_frame_no < TRANSITION_FRAMES_DOWN )
                {
                    ...
                    Debug.Assert( ind >= 0 );
                    Debug.Assert( ind < TRANSITION_INT_NUM );
        ...
        if( psLP.transition_frame_no > 0 )
        {
            ...
            Debug.Assert( TRANSITION_NB == 3 && TRANSITION_NA == 2 );
            BiquadAlt.SKP_Silk_biquad_alt( input,in_offset, B_Q28, A_Q28, psLP.In_LP_State, output,out_offset, frame_length );
        }
        else
        {
            /* Instead of using the filter, copy input directly to output */
            for(int i_djinn=0; i_djinn<frame_length; i_djinn++)
                output[out_offset+i_djinn] = input[in_offset+i_djinn];
        }
    }
}
