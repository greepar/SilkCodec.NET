...
internal class RangeCoder
{
     ...
     */
    internal static void SKP_Silk_range_encoder(
            SKP_Silk_range_coder_state      psRC,               /* I/O  compressor data structure                   */
            int                       data,               /* I    uncompressed data                           */
...
//        base_Q32 += ( range_Q16 * low_Q16 ) & 0xFFFFFFFFL;
        ...
        {
            /* Propagate carry input buffer */
            int bufferIx_tmp = bufferIx;
            do
            {
                bufferIx_tmp--;
                buffer[bufferIx_tmp] = unchecked((byte)(buffer[bufferIx_tmp] + 1));
...
//                range_Q16 = ( range_Q32 >> 8 );
            ...
            else
            {
                ...
                buffer[ bufferIx++ ] = unchecked((byte)(base_Q32 >>> 24));
//                base_Q32 = base_Q32 << 8;
...
//            base_Q32 = base_Q32 << 8;
     ...
     */
    internal static void SKP_Silk_range_encoder_multi(
            SKP_Silk_range_coder_state  psRC,              /* I/O  compressor data structure                   */
            int[]                       data,             /* I    uncompressed data    [nSymbols]             */
            int[][]                   prob,             /* I    cumulative density functions                */
            int                   nSymbols            /* I    number of data symbols                      */
        )
    {
        int k;
     ...
     */
    internal static void SKP_Silk_range_decoder(
            int[] data,             /* O    uncompressed data                           */
            int                                data_offset,
            SKP_Silk_range_coder_state      psRC,              /* I/O  compressor data structure                   */
            int[] prob,             /* I    cumulative density function                 */
            int                             prob_offset,
...
//TODO:                if( ( base_Q32 >> 16 ) != 0 ) {
     ...
     * @param nSymbols number of data symbols.
     */
    internal static void SKP_Silk_range_decoder_multi(
     ...
     * @param psRC compressor data structure.
     */
    internal static void SKP_Silk_range_enc_init(
     ...
     * @param bufferLength buffer length (input bytes).
     */
    internal static void SKP_Silk_range_dec_init(
            SKP_Silk_range_coder_state  psRC,              /* O    compressor data structure                   */
            byte[] buffer,           /* I    buffer for compressed data [bufferLength]   */
            int                         buffer_offset,
            int                   bufferLength        /* I    buffer length (input bytes)                    */
    )
    {
        /* check input */
        ...
        Array.Copy(buffer, buffer_offset, psRC.buffer, 0, bufferLength);
     ...
     * @param nBytes number of BYTES input stream.
     * @return returns number of BITS input stream.
     */
    internal static int SKP_Silk_range_coder_get_length(                /* O    returns number of BITS input stream            */
            SKP_Silk_range_coder_state    psRC,          /* I    compressed data structure                   */
            int[]                         nBytes         /* O    number of BYTES input stream                   */
        )
    {
        int nBits;

        /* Number of bits input stream */
        nBits = ( psRC.bufferIx << 3 ) + SKP_Silk_CLZ32((int) (psRC.range_Q16 - 1) ) - 14;

        nBytes [0] = (( nBits + 7)>> 3 );
        /* Return number of bits input bitstream */
        return nBits;
    }
     ...
     */
    internal static void SKP_Silk_range_enc_wrap_up(
...
//        base_Q24 &= 0xFFFFFFFF << ( 24 - bits_to_store );
        base_Q24 &= unchecked((int)0xFFFFFFFF << (24 - bits_to_store));
        ...
        if( (base_Q24 & 0x01000000) != 0 )
        {
            ...
            {
                bufferIx_tmp--;
                psRC.buffer[bufferIx_tmp] = unchecked((byte)(psRC.buffer[bufferIx_tmp] + 1));
        ...
        {
            psRC.buffer[ psRC.bufferIx++ ] = unchecked((byte)(base_Q24 >>> 16));
            if( bits_to_store > 8 )
            {
                if( psRC.bufferIx < psRC.bufferLength )
                {
                    psRC.buffer[ psRC.bufferIx++ ] = unchecked((byte)(base_Q24 >>> 8));
                }
            }
        }

        /* Fill up any remaining bits input the last byte with 1s */
        if( (bits_in_stream & 7) != 0 )
        {
            mask = 0xFF >> ( bits_in_stream & 7 );
            if( nBytes - 1 < psRC.bufferLength )
            {
                psRC.buffer[nBytes - 1] = unchecked((byte)(psRC.buffer[nBytes - 1] | mask));
            }
        }
    }

    /**
     * Check that any remaining bits input the last byte are set to 1.
     * @param psRC compressed data structure.
     */
    internal static void SKP_Silk_range_coder_check_after_decoding(
            SKP_Silk_range_coder_state      psRC               /* I/O  compressed data structure                   */
    )
    {
        ...
        /* Test any remaining bits input last byte */
        if( (bits_in_stream & 7) != 0 ) {
            mask = ( 0xFF >>( bits_in_stream & 7) );
