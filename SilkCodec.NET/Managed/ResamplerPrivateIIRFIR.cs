 ...
 * @author Dingxin Xu
 */
internal static class ResamplerPrivateIIRFIR
{
    /**
     * Upsample using a combination of allpass-based 2x upsampling and FIR interpolation.
     ...
     */
    internal static void SKP_Silk_resampler_private_IIR_FIR(
...
//littel-endian
        ...
        while( true )
        {
            ...
            {
                /* Upsample 2x */
                S.up2_function(S.sIIR, buf, ResamplerRom.RESAMPLER_ORDER_FIR_144, input, in_offset, nSamplesIn);
            }
            else
            {
                /* Fourth-order ARMA filter */
                ResamplerPrivateARMA4.SKP_Silk_resampler_private_ARMA4( S.sIIR,0, buf,ResamplerRom.RESAMPLER_ORDER_FIR_144, input,in_offset, S.Coefs,0, nSamplesIn );
            }

            max_index_Q16 = nSamplesIn << ( 16 + S.input2x );         /* +1 if 2x upsampling */

            /* Interpolate upsampled signal and store input output array */
            for( index_Q16 = 0; index_Q16 < max_index_Q16; index_Q16 += index_increment_Q16 )
            {
                ...
                output[out_offset++] = (short)SigProcFIX.SKP_SAT16( SigProcFIX.SKP_RSHIFT_ROUND( res_Q15, 15 ) );
            }
...
//little-endian
            ...
            S.sFIR[i_djinn] |= (((buf[(nSamplesIn << S.input2x) + 2*i_djinn + 1 ]>>8)&0xFF)<<24)&unchecked((int)0xFF000000);
        }
    }
}
