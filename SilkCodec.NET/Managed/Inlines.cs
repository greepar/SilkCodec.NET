...
{
    internal const int SKP_SIN_APPROX_CONST0 =      1073735400;
    internal const int SKP_SIN_APPROX_CONST1 =       -82778932;
    internal const int SKP_SIN_APPROX_CONST2 =         1059577;
    internal const int SKP_SIN_APPROX_CONST3 =           -5013;
    /**
     * count leading zeros of long.
     * @param input. input
     * @return
     */
    internal static int SKP_Silk_CLZ64(long input)
    {
        unchecked
        {
        return (int)BitOperations.LeadingZeroCount((ulong)(input));
            }
}
     ...
     * @param frac_Q7
     */
    internal static void SKP_Silk_CLZ_FRAC(int input,            /* I: input */
                                  int[] lz,          /* O: number of leading zeros */
                                  int[] frac_Q7)     /* O: the 7 bits right after the leading one */
    {
        int lzeros = (int)BitOperations.LeadingZeroCount((uint)(input));

        lz[0] = lzeros;
        frac_Q7[0] = SigProcFIX.SKP_ROR32(input, 24 - lzeros) & 0x7f;
    }
     ...
     * @return
     */
    internal static int SKP_Silk_SQRT_APPROX(int x)
    {
        unchecked
        {
        int y;
     ...
     * @return
     */
    internal static int SKP_Silk_norm16(short a)
    {
        unchecked
        {
      int a32;
      ...
      return (int)BitOperations.LeadingZeroCount((uint)(a32)) - 17;
            }
}
     ...
     * @return
     */
    internal static int SKP_Silk_norm32(int a)
    {
        unchecked
        {
      ...
      return (int)BitOperations.LeadingZeroCount((uint)(a)) - 1;
            }
}

    /**
     * Divide two int32 values and return result as int32 input a given Q-domain.
     ...
     */
    internal static int SKP_DIV32_varQ         /* O    returns a good approximation of "(a32 << Qres) / b32" */
    (
        int        a32,         /* I    numerator (Q0)                  */
        int        b32,         /* I    denominator (Q0)                */
    ...
    )
    {
        ...
        a_headrm = (int)BitOperations.LeadingZeroCount((uint)( Math.Abs(a32)) ) - 1;
        a32_nrm = a32<<a_headrm;                                    /* Q: a_headrm                    */
        b_headrm = (int)BitOperations.LeadingZeroCount((uint)( Math.Abs(b32)) ) - 1;
        b32_nrm = b32<<b_headrm;                                    /* Q: b_headrm                    */

        /* Inverse of b32, with 14 bits of precision */
        b32_inv = (int.MaxValue >> 2) / (b32_nrm>>16) ;  /* Q: 29 + 16 - b_headrm        */
    ...
    /**
     * Invert int32 value and return result as int32 input a given Q-domain.
     * @param b32 denominator (Q0)
     * @param Qres Q-domain of result (> 0)
     * @return returns a good approximation of "(1 << Qres) / b32"
     */
    internal static int SKP_INVERSE32_varQ         /* O    returns a good approximation of "(1 << Qres) / b32" */
    (
        int        b32,             /* I    denominator (Q0)                */
        int        Qres             /* I    Q-domain of result (> 0)        */
    )
     ...
     */
    internal static int SKP_Silk_SIN_APPROX_Q24(        /* O    returns approximately 2^24 * sin(x * 2 * pi / 65536) */
                                        int        x)
    {
        unchecked
        {
        ...
        /* Split range input four quadrants */
        if( x <= 32768 )
        {
            if( x < 16384 )
            {
            ...
            x = SKP_SMULWB( x<<8 , x );        /* contains x^2 input Q20 */
     ...
     */
    internal static int SKP_Silk_COS_APPROX_Q24(        /* O    returns approximately 2^24 * cos(x * 2 * pi / 65536) */
                                        int        x)
    {
        unchecked
        {
