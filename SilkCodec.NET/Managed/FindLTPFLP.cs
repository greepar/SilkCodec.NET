 ...
 * @author Dingxin Xu
 */
internal static class FindLTPFLP
{
    /**
     *
     * @param b LTP coefs.
     ...
     */
    internal static void SKP_Silk_find_LTP_FLP(
        float[] b,                      /* O    LTP coefs                               */
        float[] WLTP,                   /* O    Weight for LTP quantization             */
        float[] LTPredCodGain,          /* O    LTP coding gain                         */
        float[] r_first,          /* I    LPC residual, signal + state for 10 ms  */
        float[] r_last,           /* I    LPC residual, signal + state for 10 ms  */
        int   r_last_offset,
        int[] lag,           /* I    LTP lags                                */
...
//            w[ k ] = matrix_ptr( WLTP_ptr, LTP_ORDER / 2, LTP_ORDER / 2, LTP_ORDER );
        ...
        if( LTPredCodGain != null ) {
            ...
            EncoderCompat.Assert( LPC_LTP_res_nrg > 0 );
        ...
        for( k = 0; k < NB_SUBFR; k++ ) {
            ...
            temp = 0;
            for( i = 0; i < LTP_ORDER; i++ ) {
                delta_b[ i ] = Math.Max( b_ptr[ b_ptr_offset + i ], 0.1f );
                temp += delta_b[ i ];
            }
            temp = g / temp;
            for( i = 0; i < LTP_ORDER; i++ ) {
                b_ptr[ b_ptr_offset + i ] += delta_b[ i ] * temp;
            }
            b_ptr_offset += LTP_ORDER;
        }
    }
