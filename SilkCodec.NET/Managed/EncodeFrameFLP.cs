 ...
 * @author Dingxin Xu
 */
internal static class EncodeFrameFLP
{
    /**
     * TODO: TEST
     */
     ...
     * @return
     */
    internal static int SKP_Silk_encode_frame_FLP(
    ...
    )
    {
        ...
        if (HIGH_PASS_INPUT) {
            ...
            Array.Copy(pIn, pIn_offset, pIn_HP, 0, psEnc.sCmn.frame_length);
        }
...
/*TEST END****************************************************************************/
        if (SWITCH_TRANSITION_FILTERING != 0) {
            ...
            Array.Copy(pIn_HP, 0, pIn_HP_LP, 0, psEnc.sCmn.frame_length);
        }
...
/*TEST END****************************************************************************/
        ...
        if( psEnc.sCmn.nFramesInPayloadBuf * FRAME_LENGTH_MS >= psEnc.sCmn.PacketSize_ms ) {
            ...
            if( pnBytesOut[0] >= nBytes[0] ) {
                ...
                Array.Copy(psEnc.sCmn.sRC.buffer, 0, pCode, pCode_offset, nBytes[0]);
                ...
                if( frame_terminator > SKP_SILK_MORE_FRAMES &&
                    ...
                    Array.Copy(psEnc.sCmn.LBRR_buffer[ LBRR_idx ].payload, 0,
     ...
     */
    internal static void SKP_Silk_LBRR_encode_FLP(
        SKP_Silk_encoder_state_FLP      psEnc,             /* I/O  Encoder state FLP                       */
    ...
    )
    {
        ...
        if( psEnc.sCmn.LBRR_enabled != 0 ) {
            /* Save original gains */
            Array.Copy(psEncCtrl.sCmn.GainsIndices, 0, TempGainsIndices, 0, NB_SUBFR);
            Array.Copy(psEncCtrl.Gains, 0, TempGains, 0, NB_SUBFR);
...
//TODO:use clone rather than memory copy.
                ...
                EncoderCompat.Fill(psEnc.sCmn.q_LBRR, (byte)0);
            ...
            if( SKP_SMULBB( nFramesInPayloadBuf, FRAME_LENGTH_MS ) >= psEnc.sCmn.PacketSize_ms ) {
                ...
                if( pnBytesOut[0] >= nBytes[0] ) {
                    ...
                    Array.Copy(psEnc.sCmn.sRC_LBRR.buffer, 0, pCode, 0, nBytes[0]);
            ...
            Array.Copy(TempGainsIndices, 0, psEncCtrl.sCmn.GainsIndices, 0, NB_SUBFR);
            Array.Copy(TempGains, 0, psEncCtrl.Gains, 0, NB_SUBFR);
