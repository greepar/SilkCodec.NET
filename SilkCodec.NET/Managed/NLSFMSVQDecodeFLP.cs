 ...
 * @author Dingxin Xu
 */
internal static class NLSFMSVQDecodeFLP
{
    /**
     * NLSF vector decoder.
     * @param pNLSF
     ...
     * @param LPC_order
     */
    internal static void SKP_Silk_NLSF_MSVQ_decode_FLP(
    ...
    )
    {
        ...
        System.Diagnostics.Debug.Assert( 0 <= NLSFIndices[ NLSFIndices_offset+0 ] && NLSFIndices[ NLSFIndices_offset+0 ] < psNLSF_CB_FLP.CBStages[ 0 ].nVectors );
        ...
        Array.Copy(pCB_element, pCB_element_offset, pNLSF, 0, LPC_order);

        for( s = 1; s < psNLSF_CB_FLP.nStages; s++ ) {
            /* Check that each index is within valid range */
            System.Diagnostics.Debug.Assert( 0 <= NLSFIndices[ NLSFIndices_offset+s ] && NLSFIndices[ NLSFIndices_offset+s ] < psNLSF_CB_FLP.CBStages[ s ].nVectors );
