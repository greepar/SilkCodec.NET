 ...
 * @author Dingxin Xu
 */
internal static class AllpassIntFLP
{
    /**
     * first-order allpass filter.
...
//TODO:float or double ???
    internal static void SKP_Silk_allpass_int_FLP
    (
        float[] input,        /* I:   input signal [len]          */
        int in_offset,
        float[] S,         /* I/O: state [1]                   */
        int S_offset,
        float             A,         /* I:   coefficient (0 <= A < 1)    */
        float[] output,       /* O:   output signal [len]         */
        int out_offset,
    ...
    )
    {
        ...
        for ( k = len-1; k >= 0; k-- )
        {
            Y2        = input[in_offset] - S0;
            X2        = Y2 * A;
            output[out_offset++]  = S0 + X2;
            S0        = input[in_offset++] + X2;
        }
        S[ S_offset ] = S0;
    }
}
