...
internal static class EncAPI
{
     ...
     * @return
     */
    internal static int SKP_Silk_SDK_QueryEncoder
    (
     ...
     * @return
     */
    internal static int SKP_Silk_SDK_InitEncoder
    (
     ...
     * @return
     */
    internal static int SKP_Silk_SDK_Encode
    (
        Object                         encState,      /* I/O: State                                           */
    ...
    )
    {
        ...
        ArgumentNullException.ThrowIfNull(encControl);
        ...
        while( true )
        {
            ...
            if( API_fs_Hz == SKP_SMULBB( 1000, psEnc.sCmn.fs_kHz ) )
            {
                ...
                /* Copy to buffer */
                Array.Copy(samplesIn, samplesIn_offset, psEnc.sCmn.inputBuf, psEnc.sCmn.inputBufIx, nSamplesFromInput);
            }
            else
            {
...
//                            nBytesOut, psEnc.sCmn.inputBuf, psEnc.sCmn.inputBufIx) ) != 0 )
                    ...
                    EncoderCompat.Assert( nBytesOut[0] == 0 );
                }
