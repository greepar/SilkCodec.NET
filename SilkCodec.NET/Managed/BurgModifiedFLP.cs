...
{
    internal const int MAX_FRAME_SIZE = 544; // subfr_length * nb_subfr = ( 0.005 * 24000 + 16 ) * 4 = 544
     ...
     * @return
     */
    internal static float SKP_Silk_burg_modified_FLP(     /* O    returns residual energy                                         */
    ...
    )
    {
        ...
        Array.Copy(C_first_row, 0, C_last_row, 0, SigProcFIX.SKP_Silk_MAX_ORDER_LPC);
        ...
        for( n = 0; n < D; n++ ) {
            ...
            EncoderCompat.Assert( nrg_f > 0.0 );
            EncoderCompat.Assert( nrg_b > 0.0 );

            /* Calculate the next order reflection (parcor) coefficient */
            rc = -2.0 * num / ( nrg_f + nrg_b );
            EncoderCompat.Assert( rc > -1.0 && rc < 1.0 );
