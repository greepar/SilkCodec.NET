 ...
 * @author Dingxin Xu
 */
internal static class BwexpanderFLP
{
    /**
     ...
     * @param chirp chirp factor (typically input range (0..1) ).
     */
    internal static void SKP_Silk_bwexpander_FLP(
        float[] ar,        /* I/O  AR filter to be expanded (without leading 1)    */
        int ar_offset,
        int       d,          /* I    length of ar                                    */
        float     chirp       /* I    chirp factor (typically input range (0..1) )       */
    )
    {
        int   i;
        float cfac = chirp;
