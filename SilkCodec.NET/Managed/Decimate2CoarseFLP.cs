 ...
 * @author Dingxin Xu
 */
internal static class Decimate2CoarseFLP
{
    /* coefficients for coarser 2-fold resampling */
    internal static float[] A20c_FLP = {0.064666748046875f, 0.508514404296875f};
    internal static float[] A21c_FLP = {0.245666503906250f, 0.819732666015625f};
     ...
     */
    internal static void SKP_Silk_decimate2_coarse_FLP
    (
    ...
    )
    {
        ...
        for ( k = 0; k < len; k++)
        {
            scratch[ scratch_offset + k ]       = input[ in_offset + 2 * k ];
        ...
        for ( k = 0; k < len; k++ )
        {
            output[ out_offset+k ] = 0.5f * ( scratch[ scratch_offset+k ] + scratch[ scratch_offset + k + len ] );
        }
    }
}
