 ...
 * @author Dingxin Xu
 */
internal static class ResamplerPrivateUp4
{
    /**
     * Upsample by a factor 4.
     * Note: very low quality, only use with output sampling rates above 96 kHz.
     ...
     * @param output Output signal [ 4 * len ].
     * @param out_offset offset of valid data.
     * @param input Input signal [ len ].
     * @param in_offset offset of valid data.
     * @param len Number of INPUT samples.
     */
    internal static void SKP_Silk_resampler_private_up4(
        int[] S,             /* I/O: State vector [ 2 ]                      */
        int S_offset,
        short[] output,           /* O:   Output signal [ 4 * len ]               */
        int out_offset,
    ...
    )
    {
        ...
        Debug.Assert( ResamplerRom.SKP_Silk_resampler_up2_lq_0 > 0 );
        Debug.Assert( ResamplerRom.SKP_Silk_resampler_up2_lq_1 < 0 );
        ...
        for( k = 0; k < len; k++ )
        {
            ...
            output[ out_offset + 4 * k ]     = (short)out16;
            output[ out_offset + 4 * k + 1 ] = (short)out16;
            ...
            /* Convert back to int16 and store to output */
            out16 = (short)SigProcFIX.SKP_SAT16( SigProcFIX.SKP_RSHIFT_ROUND( out32, 10 ) );
            output[ out_offset + 4 * k + 2 ] = (short)out16;
            output[ out_offset + 4 * k + 3 ] = (short)out16;
        }
    }
}
